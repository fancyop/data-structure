...
void inset_random(void)
{
	...
	scanf("%d", &item);
	ptr->data = item;
	printf("Enter the location(>=0) of the node to perform the insertion after this node: ");
	scanf("%d", &loc);
...
void display(void)
{
	...
	printf("Display the entire singly linked list:\n");
	if(head == NULL){
...
void search(void)
{
	...
	while(temp != NULL){
		if(temp->data == item){
			printf("Find the target item at location %d", loc);
			flag = 1;
		}
		temp = temp->next;
		loc++;
